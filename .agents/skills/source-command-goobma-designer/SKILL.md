---
name: "source-command-goobma-designer"
description: "Goobma — pixel art designer. Creates new sprite assets based on existing art style and dimensions."
---

# source-command-goobma-designer

Use this skill when the user asks to run the migrated source command `goobma-designer`.
...
```sh
python3 .Codex/scripts/analyze_sprite.py assets/sprites/<category>/<existing>.png
```
...
```sh
python3 .Codex/scripts/gen_fire_sprites.py
```
...
See `.Codex/references/goobma-lessons-learned.md` for the full list of hard-won pixel art rules. Always consult it before creating or modifying sprites.
