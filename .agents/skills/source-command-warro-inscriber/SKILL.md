---
name: "source-command-warro-inscriber"
description: "Warro — the Inscriber. Audits the codebase and updates all documentation to match the current state of the code."
---

# source-command-warro-inscriber

Use this skill when the user asks to run the migrated source command `warro-inscriber`.
...
```sh
python3 .Codex/scripts/analyze_sprite.py assets/sprites/<category>/<sprite>.png [frame_w] [frame_h]
```
...
python3 .Codex/scripts/analyze_sprite.py assets/sprites/player/player.png

# Verify spider uses 64×48 frames as documented
python3 .Codex/scripts/analyze_sprite.py assets/sprites/entities/spider.png 64 48

# Check all entity sprites in one pass
for f in assets/sprites/entities/*.png; do
    python3 .Codex/scripts/analyze_sprite.py "$f" 2>&1 | grep -E "Sheet size|Frame size|Grid|Envelope"
done
...
for f in $(find assets/sprites -name "*.png" | sort); do
    ...
    python3 .Codex/scripts/analyze_sprite.py "$f" 2>&1 | grep -E "Sheet size|Frame size|Grid"
done
```
...
| `.Codex/references/entity-template.md` | Template for adding new entities |
| `.Codex/references/coding-standards.md` | Comment style and coding conventions |
| `.Codex/references/animation-sequences.md` | Sprite animation state machines and timing |
| `.Codex/references/sprite-sheet-analysis.md` | Sprite sheet measurement and analysis guide |
| `.Codex/commands/*.md` | Slash command definitions (Bosser, Lugio, Goobma, Warro) |
...
- **Editor:** scan `src/editor/` for all modules (editor, canvas, palette, properties, tools, ui, undo, serializer, file_dialog)
...
- **Sprite analysis:** run `analyze_sprite.py` on key sprites to verify documented dimensions:
  ...
  for f in assets/sprites/player/*.png assets/sprites/entities/*.png assets/sprites/hazards/*.png; do
      echo "--- $(basename $f) ---"
      python3 .Codex/scripts/analyze_sprite.py "$f" 2>&1 | grep -E "Sheet size|Frame size|Grid|Envelope|Origin|Size"
  done
  ```
...
### Step 3: Cross-reference .Codex/references/ against code
...
python3 .Codex/scripts/analyze_sprite.py assets/sprites/player/player.png 48 48
python3 .Codex/scripts/analyze_sprite.py assets/sprites/entities/spider.png 64 48
python3 .Codex/scripts/analyze_sprite.py assets/sprites/entities/bird.png
python3 .Codex/scripts/analyze_sprite.py assets/sprites/hazards/blue_flame.png
python3 .Codex/scripts/analyze_sprite.py assets/sprites/surfaces/bouncepad_small.png
```
...
- **`.Codex/commands/bosser-engineer.md`** (Bosser, the engineer):
...
- **`.Codex/commands/lugio-creator.md`** (Lugio, the level builder):
...
- **`.Codex/commands/goobma-designer.md`** (Goobma, the pixel art designer):
...
- **`.Codex/commands/warro-inscriber.md`** (Warro, that's you):
...
See `.Codex/references/warro-lessons-learned.md` for the full list of hard-won documentation rules. Always consult it before auditing or updating docs.
