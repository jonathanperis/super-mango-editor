/* Ordered semantic commands share one route/settings/inspector ownership path. */
...
static void request_terminal_action(GameState *gs, GameTerminalAction action)
{
    ...
    } else gs->route = game_terminal_action_route(action);
}

static void sync_pause_music(GameState *gs)
{
    if (game_overlay_state(gs) == GAME_OVERLAY_PAUSED) music_pause();
    else { music_resume(); gs->loop.prev_ticks = clock_millis(); }
}
...
void game_handle_events(GameState *gs)
{
    ...
    while (input_poll(&event)) {
        ...
        if (event.type == INPUT_QUIT) {
            ...
            if (gs->controller == event.device) { gs->controller = 0; game_input_clear_controller_latch(gs); }
