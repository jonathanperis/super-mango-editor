/* CLI parsing only; AppSession owns raylib, the frame loop and teardown. */
#include <errno.h>
#include <limits.h>
#include <stdio.h>
#include <stdlib.h>
...
{
    if (*index + 1 >= argc || !argv[*index+1][0] || argv[*index+1][0] == '-') return NULL;
    return argv[++*index];
}
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(option, "--debug")) config.debug_mode = 1;
        else if (!strcmp(option, "--sandbox")) config.level_path = "levels/00_sandbox_01.toml";
        else if (!strcmp(option, "--no-save")) config.profile_enabled = 0;
        else if (!strcmp(option, "--continue")) config.continue_last = 1;
            ...
            if (!value) { fprintf(stderr, "Error: %s requires a path\n", option); return EXIT_FAILURE; }
            if (!strcmp(option, "--level")) config.level_path = value;
            else if (!strcmp(option, "--profile")) config.profile_path = value;
            else if (!strcmp(option, "--replay-script")) config.replay_script_path = value;
            else { config.experiment_path = value; config.debug_mode = 1; config.profile_enabled = 0; }
        } else if (!strcmp(option, "--seed") || !strcmp(option, "--smoke-test-frames")) {
            unsigned int value;
            const char *text = argument(argc, argv, &i);
            if (unsigned_argument(text, &value)) { fprintf(stderr, "Error: %s requires an unsigned integer\n", option); return EXIT_FAILURE; }
            if (!strcmp(option, "--seed")) { config.random_seed = value; seed_set = 1; }
            else {
                if (!value || value > INT_MAX) { fprintf(stderr, "Error: --smoke-test-frames requires a positive integer\n"); return EXIT_FAILURE; }
    ...
    if (config.smoke_test_frames > 0 && !config.level_path) config.level_path = "levels/00_sandbox_01.toml";
