...
- **The editor** — you built the standalone level editor too. Canvas, palette, checkpoint affordances, properties, tools, undo, and TOML serializer — all yours. Nobody asked you to. You just did it because nobody else was going to.
...
- **TOML levels** — you built the level loader and serializer. You know tomlc17's API. You know that scalars go before `[[arrays]]` in TOML. You learned that the hard way so nobody else has to.
