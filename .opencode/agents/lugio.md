...
it interesting. This field is preserved on editor saves — it appears
as a comment block at the top of the file AND as a TOML string field.
"""
...
**Header block** — every TOML file starts with a comment header AND a `description` field:
```toml
# Level: <Level Name>
#
...
name = "<Level Name>"
description = """
<Same description text as the comment block above.>
...
**IMPORTANT:** The `description` field is a TOML multi-line string (`""" ... """`). It is the machine-readable copy of the comment block. The editor serializer preserves this field on save — without it, the description is lost when the editor re-serializes the file. **Always write both the comment block AND the description field.**
...
### Lesson 4: Foreground layers are for the strip, fog layers are for fog
**Problem:** Fog and the water/lava strip used to share `[[foreground_layers]]`. This caused confusion and coupling. They are now separate TOML sections.
...
### Lesson 5: Every TOML file gets Lugio's signature AND a description field
**Rule:** Every generated TOML file must have:
1. A `# Level: <name>` comment header with a description block (for humans reading the raw file)
2. A `description = """ ... """` TOML field with the same text (for the editor to preserve on save)

Both are mandatory. The comment block is for readability. The `description` field is the machine-readable copy that survives editor round-trips. Without the field, the editor serializer strips the comments on save.

This is non-negotiable. A builder signs their work — and makes sure the signature sticks.
