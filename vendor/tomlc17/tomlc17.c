...
static char *cell_realloc(char *p, int size) {
  ...
  int64_t grown = (int64_t)size * 13 / 10 + 100;
  int newmax = grown > INT_MAX ? INT_MAX : (int)grown;
...
#define ARRAY_MAX 36000 // preserve the project's maximum experiment frame count
