...
Current numeric facts come from `docs/src/generated/project.json`, generated by
`make content-inventory`. The [Level Catalog](docs/wiki/level-catalog.md) and
[Asset Inventory](docs/wiki/asset-inventory.md) contain the corresponding detail.
The project now includes Sandbox School, focused mechanics levels and an opt-in
simulation inspector with reproducible experiment capture.
...
- Campaign names: Creator's Playground, Volcanic Depths 1, Volcanic Depths 2.
...
**Historical design assessment:** this section and the design/A-B proposals below
describe an earlier website. They are planning context, not current-state audit
findings or authoritative content counts. Current navigation, accessibility and
content facts must be checked against source and generated outputs.
...
For the sandbox-school goal, prioritize successful first build, first edit and
playtest, ability to locate a behavior's source, completion of the entity lab,
